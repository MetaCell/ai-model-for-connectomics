...
# Running inference using the FAFB Nuroglancer container

1. Start mongo db using the docker compose configuration in mongo-setup:
`docker compose up`
2. Import synapses data in mongo as described above in Usage section 0.
3. Run `build/lib/synister/predict_pipeline.py` in this repository. Note that this requires weights trained on the same container data for accurate predictions.